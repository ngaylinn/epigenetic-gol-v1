...
__device__ __host__ void select(
    ...
    // they're all equally fit, just pick each individual one time.
    if (total_fitness == 0) {
        for (int i = 0; i < batch_size; i++) {
    ...
    // This variable is an index into the batch of organisms to select from /
    // the wedges of the roulette wheel. Starting at -1 indicates we have not
    // yet reached a sample point inside the 0th wedge, but will do so in the
...
__global__ void SelectKernel(
        ...
        curandState* rngs) {
    // Divide the population into batches of num_organisms each.
